...
linking = pd.read_excel("linking/bank_llinking_10_freq_treshold.xlsx")
...
path_to_files = "data/result_tables/described/"
files = os.listdir(path_to_files)

full_df_analyst = []
for f in files:
    df = pd.read_excel(path_to_files+f)
...
for i in range(46):
    ...
    sa_part["recdet_analyst"] = sa_part.apply(lambda x: get_ranalyst(x[6], [x[3], x[4], x[5]]), axis=1)
    sa_part["recdet_bank"] = sa_part.apply(lambda x: get_rbank(x[6], [x[3], x[4], x[5]]), axis=1)
    print(start_index, end_index, datetime.now() - nowt)

    sa_part.to_excel("data/linking_result/" + "linked_analysts_" + str(start_index) + "_" + str(end_index) + ".xlsx")
...
for i in range(46):
    ...
    df = pd.read_excel("data/linking_result/" + "linked_analysts_" + str(start_index) + "_" + str(end_index) + ".xlsx")
